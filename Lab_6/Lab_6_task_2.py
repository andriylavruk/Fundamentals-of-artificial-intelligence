...
def strategy2(b=s3.body):
    u"""Стратегія робота, який сканує сектор ультразвуковим сенсором, з реалізацією 
    найпростішого машинного навчаннм з підкріпленням (Q-learning). Кожні 10 кадрів 
    визначається чи обєкти s1 або s2 знаходяться в межах сектора. Якщо це s1 
    то стан=об'єкт. Якщо це s2 то стан= антиоб'єкт. Якщо нічого, то стан=нічого.  
    Установлюється стан і винагорода. В Q-таблиці (див.) оновлюється сума винагород, 
    яка відповдає стану і дії. Дія 0 - залишати напрямок, 1 - змінювати (випадковий кут). 
    Далі алгоритм вибирає дію. З заданими імовірностями дія може бути випадковою або 
    відповідати дії з максимальною сумою винагород для цього стану (оптимальною). 
    Далі дія виконується. Вкінці алгоритм запобігає виїзду робота за межі кола."""
    v=100
    a=b.angle
    b.velocity=v*cos(a), v*sin(a)
    x,y=b.position
...
def strategy3(b=s3.body):
    u"""Стратегія робота з Q-навчанням та ε-greedy підходом зі змінним ε"""
    v = 100
    a = b.angle
    b.velocity = v*cos(a), v*sin(a)
    x, y = b.position
    ...
    if canvas.frame % 5 == 0:  # Кожні 5 кадрів
        inS = inSector(s1.body.position[0], s1.body.position[1], x, y, 100, a)
        inS2 = any(inSector(s.body.position[0], s.body.position[1], x, y, 100, a) for s in S2)
        ...
        # Зменшення epsilon з часом
        if not hasattr(b, 'epsilon'):
            b.epsilon = 0.5  # Початкове значення
        b.epsilon = max(0.2, b.epsilon * 0.999)  # Зменшення, але не менше 0.2
        
        # ε-greedy стратегія вибору дії
        if random.random() < b.epsilon:
            b.action = random.choice([0, 1])  # Випадковий вибір
        else:
            b.action = np.argmax(b.Q[state])  # Вибір найкращої дії

        print(state, b.action, b.Q, b.epsilon)

        if b.action:  # Якщо змінювати напрямок
            b.angle += random.uniform(-0.7, 0.7)  # Менш різкі повороти  
