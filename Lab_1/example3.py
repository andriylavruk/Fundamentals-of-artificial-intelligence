...
def rule3(d, p):
    """Правило для об'єднання методів обробки, які мають спільні переваги.
    (x, "перевага", z) & (y, "перевага", z) -> (x, "схожий на", y)"""
    if "перевага" not in d:
        return False
    l = len(d)
    c = d["перевага"].copy()
    for t1 in c:
        for t2 in c:
            if t1[2] == t2[2] and t1[0] != t2[0]:  # спільна перевага і різні методи
                if p not in d:
                    d["схожий на"] = set()  # створити новий предикат, якщо його ще нема
                d["схожий на"].add((t1[0], "схожий на", t2[0]))
    return len(d) != l
